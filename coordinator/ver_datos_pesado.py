"""
Utilidad para ver y exportar los datos de OS Grid — Prueba pesada (3 chunks)

Copia de ver_datos.py apuntando a la base de datos separada de la prueba
pesada (os_grid_pesado.db), para no mezclar el análisis con las corridas de
la prueba original.

Uso (desde la carpeta os-grid, con el venv activo):

    python -m coordinator.ver_datos_pesado              # lista todas las corridas
    python -m coordinator.ver_datos_pesado 1             # detalle de la corrida #1
    python -m coordinator.ver_datos_pesado 1 --csv       # exporta la corrida #1 a CSV

El CSV se guarda como  corrida_pesada_<id>.csv  en la carpeta del proyecto.
"""
...
def _con():
    if not RUTA_DB.exists():
        print("Todavía no hay base de datos de la prueba pesada. Corré una misión primero.")
        sys.exit(1)
...
def listar_corridas():
    ...
    print("\nUsá  python -m coordinator.ver_datos_pesado <id>  para ver el detalle.\n")
...
def detalle_corrida(cid: int):
    ...
    print(f"\n=== Corrida pesada #{cid} — comparativa por sistema operativo ===\n")
...
def exportar_csv(cid: int):
    ...
    salida = RUTA_DB.parent / f"corrida_pesada_{cid}.csv"
