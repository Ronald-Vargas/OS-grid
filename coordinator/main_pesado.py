"""
Coordinador de OS Grid — Prueba pesada (3 chunks)

Copia de coordinator/main.py con una configuración distinta de la misión:
en vez de 12 chunks livianos, reparte solo 3 chunks mucho más grandes, para
que la carga sostenida por máquina sea mayor y la diferencia de rendimiento
entre Linux/macOS/Windows se note más. Toda la lógica del servidor (registro
de workers, reparto dinámico de la cola, métricas en vivo, resumen final,
dashboard) es idéntica a main.py; lo único que cambia es la configuración de
la tarea y que los resultados se guardan en una base de datos separada
(os_grid_pesado.db, vía stats_pesado.py) para no mezclarse con la corrida
original.

Como corre en el mismo puerto (8000) que main.py, no se ejecutan los dos
coordinadores al mismo tiempo: se para uno (Ctrl+C) antes de levantar el
otro. Los workers (worker/agent.py) no necesitan ningún cambio para hablar
con este coordinador: mismo protocolo, mismo puerto.
...
from coordinator import stats_pesado as stats  # noqa: E402

# ------------------------------------------------------------------
# Configuración de la tarea global a distribuir (versión pesada).
# ------------------------------------------------------------------
# Mismo número secreto que la prueba original, para mantener la
...
RANGO_TOTAL = 45_000_000  # revisamos del 0 al 45.000.000 (vs 12M en la prueba original)
NUM_CHUNKS = 3             # partido en solo 3 pedazos, ~15.000.000 números cada uno
                           # (~24s por chunk, ~15x más pesado que los de la prueba
                           # original, para que la carga sostenida por máquina sea
                           # mucho más notoria)
...
app = FastAPI(title="OS Grid Coordinator — Prueba Pesada")
...
def estado_api():
    return {
        "servicio": "OS Grid Coordinator — Prueba Pesada",
        "objetivo": OBJETIVO,
...
async def ws(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            if tipo == P.REGISTER:
                ...
                if estado.corrida_id is None:
                    ...
                    print(f"[COORD-PESADO] Corrida #{estado.corrida_id} registrada en la base de datos")
                print(f"[COORD-PESADO] Worker registrado: {nombre} "
                ...
                print(f"[COORD-PESADO] Workers conectados: {n}/{MIN_WORKERS}")
                await estado.broadcast({
                    "type": "waiting",
                    "conectados": n,
                    "minimo": MIN_WORKERS,
                ...
                if n >= MIN_WORKERS and not estado.arranque.is_set():
                    print("[COORD-PESADO] ¡Todos los workers listos! Arrancando misión...")
                    estado.arranque.set()
            ...
            # --- RESULT: el worker terminó un chunk ---
                ...
                print(f"[COORD-PESADO] {nombre} terminó chunk #{cid} "
        ...
        if nombre:
            print(f"[COORD-PESADO] Worker desconectado: {nombre}")
...
async def asignar(websocket: WebSocket, nombre: str):
    ...
    if chunk is None:
        ...
        return
    print(f"[COORD-PESADO] -> {nombre}: asignando chunk #{chunk['id']} "
...
def resumen():
    print("\n" + "=" * 52)
    print(" MISIÓN COMPLETA (PRUEBA PESADA)")
    print("=" * 52)
    ...
    print(f"\n  Datos guardados en: os_grid_pesado.db (corrida #{estado.corrida_id})")
    print("=" * 52 + "\n")
...
async def ws_dashboard(websocket: WebSocket):
    ...
    print(f"[COORD-PESADO] Dashboard conectado ({len(estado.dashboards)} activos)")
    ...
    try:
        ...
        print(f"[COORD-PESADO] Dashboard desconectado ({len(estado.dashboards)} activos)")


# Servir el dashboard Angular ya compilado (cuando exista la carpeta static).
# Reutiliza el mismo build que coordinator/main.py (el dashboard es agnóstico
# a cuántos chunks tenga la misión).
...
if (_static / "index.html").exists():
    ...
    def sin_dashboard():
        return (
            "<h2>OS Grid Coordinator — Prueba Pesada</h2>"
